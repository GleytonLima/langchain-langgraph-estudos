...
run_history/manifest, que substituímos por stubs. O foco é a fiação HTTP (rotas,
corpo, response_model) — a lógica de cada função já é testada em isolado.
"""
...
def test_lifespan_tolera_falha_no_warmup(monkeypatch):
    # warmup é best-effort: se a infra não subir, o app ainda inicia.
    async def boom():
