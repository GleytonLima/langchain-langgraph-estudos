...
def test_field_options_vazio_para_tool_sem_referencias():
    assert rt._field_options("listar_categorias") == {}


def test_field_options_traz_dois_seletores_de_abrir():
    out = rt._field_options("abrir_chamado")
    assert set(out) == {"categoria_id", "prioridade"}
    assert all(hasattr(o, "value") and hasattr(o, "label") for opts in out.values() for o in opts)
